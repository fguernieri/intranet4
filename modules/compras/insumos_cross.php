...
<body class="bg-gray-900 text-gray-100 flex min-h-screen">
  <main class="flex-1 bg-gray-900 p-6 relative">
    ...
    <form id="pedido-form" action="salvar_pedido_cross.php" method="post">
...
`;
    ...
    document.getElementById('export-pdf').onclick = ()=>{
      ...
      html2pdf().set({
        margin:0.5, filename:'pedido_insumos_cross.pdf',
        html2canvas:{scale:2},
    ...
    document.getElementById('confirm-preview').onclick = async ()=>{
      ...
      // Filial e usuário são pegos no PHP via sessão/hardcoded no script salvar_pedido_cross.php

      try {
        const response = await fetch('salvar_pedido_cross.php', {
          method: 'POST',
          body: formData
        });

        if (response.ok) {
          // O PHP fará o redirect para a URL correta, incluindo &pedido=
          // Se o fetch seguir o redirect (padrão), response.url será a URL final.
          // Se não, o navegador simplesmente seguirá o Location header do PHP.
          window.location.href = response.url.includes('status=ok') ? response.url : 'insumos_cross.php?status=ok';
        } else {

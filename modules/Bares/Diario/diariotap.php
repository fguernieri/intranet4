...
if ($ano_selecionado && $mes_selecionado && $dia_selecionado) {
    ...
    $sql_main = "
        SELECT 
            ...
            bdf.musica_banda_sempre_digitar_o_nome_dos_mesmo_jeito,
            bdf.musica_resposta_publico,
            bdf.cardapio_disponibilidade_chope,
            bdf.cardapio_temperatura_chope,
            bdf.cardapio_comida,
            bdf.cardapio_disponibilidade_comida,
            bdf.cardapio_disponibilidade_drink,
            bdf.equipe_bar,
            bdf.equipe_salao,
            bdf.equipe_cozinha,
            bdf.equipamento_funcionamento,
            bdf.equipamento_com_problema,
            bdf.sistema_funcionamento,
            bdf.sistema_com_problema,
            bdf.observacoes,
...
<head>
  ...
  <title>Diário de bordo - Bar da Fábrica</title>
...
<body class="bg-gray-900 text-gray-100 flex min-h-screen">
  ...
  <main class="flex-1 bg-gray-900 p-6 relative">
    ...
    <h1 class="text-2xl font-bold text-yellow-400 text-center mb-4"> 
      Diário de bordo - Bar da Fábrica
    </h1>
    ...
    <div class="mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
      <div class="bg-gray-800 rounded-lg shadow-md p-1 flex flex-col justify-center w-full h-28">
        <form method="POST" id="filtroForm" class="flex flex-col w-full">
          ...
          <button type="submit" id="submitBtn" class="mt-1 bg-yellow-500 text-gray-900 px-2 py-1 rounded hover:bg-yellow-600 focus:outline-none">
            Filtrar
          </button>
        </form>
      </div>
...
<?php endif; ?>
    ...
    <div class="mt-8 bg-gray-800 rounded-lg shadow-md p-4 flex flex-col items-stretch min-h-0 w-full">
      ...
      <div class="flex-1 flex items-center justify-center min-h-0" style="height:300px; position: relative;">
        <canvas id="graficoFaturamentoMes" class="w-full h-full"></canvas>
        <div id="marcadoresMedia" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none;"></div>
        <div class="absolute bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-600 rounded-bl-md px-1 py-0.5 text-xs text-gray-300 cursor-pointer hover:from-gray-700 hover:to-gray-800 hover:border-gray-500 transition-all duration-300 shadow-md" style="top: -5px; right: -5px;" onclick="openModal('diasAbaixoMediaModal')">
          <div class="text-center">
            <div class="text-red-400 font-bold text-xs"><?= htmlspecialchars($dias_abaixo_media ?? 0) ?></div>
            <div class="text-xs leading-none text-gray-300">abaixo</div>
          </div>
        </div>
      </div>
      <div class="text-center mt-3 text-sm text-gray-300 flex justify-center items-center space-x-6 bg-gray-700 py-2 px-4 rounded">
        <span class="inline-flex items-center">
          <div class="w-6 h-3 bg-yellow-400 mr-3 rounded-sm"></div>
          <span class="font-medium">FATURAMENTO</span>
        </span>
        <span class="inline-flex items-center">
          <div class="w-6 h-1 bg-red-500 mr-3"></div>
          <span class="font-medium">MÉDIA -3M</span>
        </span>
      </div>
    </div>
    <script>
      ...
      // Converter datas para garantir que estamos no mês correto
      ...
      // Médias 3m por dia da semana (passadas do PHP)
      const medias3mPorDiaSemana = <?= json_encode($medias_3m_por_dia_semana) ?>;
      
      // Criar array com as médias 3m para cada dia do mês
      const mediasPorDia = Object.keys(dadosFaturamentoMes).map(dt => {
        const [ano, mes, dia] = dt.split('-');
        const d = new Date(parseInt(ano), parseInt(mes) - 1, parseInt(dia));
        const diaSemanaNum = d.getDay() + 1; // 1=Dom ... 7=Sáb
        return medias3mPorDiaSemana[diaSemanaNum] || 0;
      });
      ...
      const chartMes = new Chart(ctxMes, {
        ...
        options: {
          ...
          plugins: {
            legend: {
              display: false  // Desabilitar legenda automática do Chart.js
            },
            tooltip: {
              callbacks: {
                ...
                label: function(context) {
                  ...
                  const media = mediasPorDia[idx].toLocaleString('pt-BR', {style:'currency',currency:'BRL'});
                  return [
                    'Faturamento: ' + fat,
                    'Média 3m (' + diasSemanaMes[idx] + '): ' + media,
                    'Banda: ' + banda
                  ];
                }
              }
          ...
          onHover: function(event, activeElements) {
            // Atualizar marcadores quando hover
            criarMarcadoresMedia();
          }
        }
      });

      // Função para criar os marcadores de média
      function criarMarcadoresMedia() {
        const canvas = chartMes.canvas;
        const rect = canvas.getBoundingClientRect();
        const chartArea = chartMes.chartArea;
        const marcadoresContainer = document.getElementById('marcadoresMedia');
        
        // Limpar marcadores existentes
        marcadoresContainer.innerHTML = '';
        
        // Criar um marcador para cada dia
        labelsDiasMes.forEach((label, index) => {
          const media = mediasPorDia[index];
          if (media > 0) {
            // Calcular posição X da barra
            const xPosition = chartArea.left + ((index + 0.5) * (chartArea.width / labelsDiasMes.length));
            
            // Calcular posição Y baseada no valor da média
            const yScale = chartMes.scales.y;
            const yPosition = yScale.getPixelForValue(media);
            
            // Criar o marcador (linha vermelha)
            const marcador = document.createElement('div');
            marcador.style.position = 'absolute';
            marcador.style.left = (xPosition - 15) + 'px';
            marcador.style.top = yPosition + 'px';
            marcador.style.width = '30px';
            marcador.style.height = '3px';
            marcador.style.backgroundColor = 'red';
            marcador.style.borderRadius = '1px';
      ...
      // Criar marcadores após o gráfico carregar
      setTimeout(criarMarcadoresMedia, 100);
    </script>
    <?php endif; ?>

    <script>
        // Remover sistema AJAX - usar apenas formulário tradicional

...
if ($ano_selecionado) {
    ...
    $sql_resp = "SELECT DATE(hora_de_inicio) AS data, musica_banda_sempre_digitar_o_nome_dos_mesmo_jeito AS banda, musica_resposta_publico AS resposta FROM DiarioDeBordoBdf WHERE YEAR(hora_de_inicio)=? AND musica_resposta_publico<> 'Bom' ORDER BY data";
    ...
    $sql_problemas = "SELECT DATE(hora_de_inicio) AS data, equipamento_com_problema, sistema_com_problema FROM DiarioDeBordoBdf WHERE YEAR(hora_de_inicio)=? AND (equipamento_funcionamento!='Funcionando normalmente' OR sistema_funcionamento!='Funcionando normalmente') ORDER BY hora_de_inicio ASC";

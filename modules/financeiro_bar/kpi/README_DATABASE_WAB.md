# 📊 Documentação de Banco de Dados - KPI WAB
...
Este sistema gerencia os **Indicadores de Performance (KPIs)** do WAB (We Are Bastards), processando dados financeiros de receitas e despesas para gerar análises mensais, comparações temporais, tendências e relatórios DRE (Demonstrativo de Resultado do Exercício).

**Arquivos principais:**
- `kpiWAB.php` - Dashboard principal
- `api/get_category_details_wab.php` - Detalhamento de categorias
- `api/get_dre_analysis_wab.php` - Análise DRE com subcategorias
- `js/kpi_details_wab.js` - Modal de detalhes interativo
...
### 1️⃣ **freceitawab** (Receitas Agregadas)
Armazena receitas mensais consolidadas do WAB.
...
// get_category_details_wab.php (linha 88-96)
...
// get_dre_analysis_wab.php (linha 45-52)
...
### 2️⃣ **fdespesaswab** (Despesas Agregadas)
Armazena despesas mensais consolidadas do WAB por categoria pai e subcategoria.
...
// get_category_details_wab.php (linha 116-124)
$all_despesas_detalhes = $supabase->select('fdespesaswab', [
...
// get_dre_analysis_wab.php (linha 55-62)
...
### 3️⃣ **fdespesaswab_detalhes** (Despesas Detalhadas)
...
### 4️⃣ **fmetaswab** (Metas e Percentuais)
...
### 📍 **GET** `/api/get_category_details_wab.php`
...
**Tabelas consultadas:**
- `freceitawab` - Receitas para cálculo de %
- `fdespesaswab` - Despesas agregadas com subcategorias

---

### 📍 **GET** `/api/get_dre_analysis_wab.php`
...
**Tabelas consultadas:**
- `freceitawab` - Receitas operacionais e não operacionais
- `fdespesaswab` - Todas as categorias de despesas com subcategorias
...
### 1. Dashboard Principal (`kpiWAB.php`)

```
┌─────────────────────────────────────────────────┐
│ 1. Carrega período disponível mais recente      │
│    SELECT data_mes FROM freceitawab             │
...
│    - fdespesaswab (despesas agregadas)          │
│    - fdespesaswab_detalhes (detalhes)           │
│    - fmetaswab (metas do período)               │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
...
│ - freceitawab (para % receita)                  │
│ - fdespesaswab (categoria_pai = CUSTO FIXO)     │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
...
│ GET /api/get_dre_analysis_wab.php?periodo=2025/11  │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│ API calcula 6 meses de histórico                │
│ - freceitawab (todas as receitas)               │
│ - fdespesaswab (todas as despesas)              │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
...
├── kpiWAB.php                    # Dashboard principal (2150 linhas)
├── api/
│   ├── get_category_details_wab.php  # Detalhes de categoria (576 linhas)
│   └── get_dre_analysis_wab.php      # Análise DRE (328 linhas)
├── js/
│   └── kpi_details_wab.js            # Modal interativo (958 linhas)
├── css/
...
**Desenvolvido para:** We Are Bastards (WAB)

...
if ($conexao_ok) {
    try {
        // Buscar todos os períodos disponíveis nas views de receita aberta e fechada
        ...
        if (empty($periodo_selecionado) && !empty($periodos_disponiveis)) {
            ...
            if (isset($periodos_disponiveis[$mes_atual])) {
                ...
                // Caso contrário, procurar o período mais recente que não seja futuro
                ...
                // Se não encontrou nenhum período passado, usar o mais antigo disponível
        ...
        if ($periodo_selecionado) {
            ...
            if (count($partes) == 2) {
                ...
                if (isset($periodos_aberto_set[$periodo_selecionado]) || $periodo_selecionado === $mes_atual) {
                    // Para o mês corrente usar a view aberta (contém lançamentos pendentes e recebidos)
                    ...
                    // Para meses passados usar a view fechada (apenas valores consolidados como recebidos)
                ...
                // Buscar despesas SEMPRE de fdespesasfabrica
                // Buscar despesas SEMPRE de fdespesasfabrica, para qualquer mês selecionado
                // Buscar despesas de qualquer mês selecionado
                ...
                // Buscar detalhes das despesas de qualquer mês selecionado
...
function listarMetasDisponiveis($periodo = null) {
    ...
    if ($periodo_busca && preg_match('/^(\d{4})\/(\d{2})$/', $periodo_busca, $matches)) {
        ...
        try {
            // Ler metas somente da tabela final (fmetasfabricafinal)
            ...
            // também definir uma flag debug global informando se as tabelas possuem registros
            $check_final = [];
            $check_old = [];
            try { $check_final = $supabase->select('fmetasfabricafinal', ['select' => 'CATEGORIA', 'limit' => 1]); } catch (Exception $e) { }
            try { $check_old = $supabase->select('fmetasfabrica', ['select' => 'CATEGORIA', 'limit' => 1]); } catch (Exception $e) { }
            $has_final = !empty($check_final) ? 1 : 0;
            $has_old = !empty($check_old) ? 1 : 0;
            $GLOBALS['METAS_TABLES_DEBUG'] = "<!-- METAS_TABLES_INDEX: fmetasfabricafinal={$has_final} fmetasfabrica={$has_old} -->";
...
function buildVistosIndex($pageSize = 10000) {
    ...
    while (true) {
        try {
            $rows = $supabase->select('fcontaspagarfabrica_vistos', [ 'select' => '*', 'limit' => $pageSize, 'offset' => $offset ]);
        } catch (Exception $e) {
...
if (!empty($_GET['debug_vistos_json']) && $_GET['debug_vistos_json'] === '1') {
    ...
    try {
        ...
        echo json_encode([ 'success' => false, 'error' => 'Erro ao carregar fcontaspagarfabrica_vistos', 'detail' => $e->getMessage() ], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        exit;
    }
...
if (!empty($_GET['debug_vistos_matches']) && $_GET['debug_vistos_matches'] === '1') {
    ...
    try {
        $detalhes = $supabase->select('fdespesasfabrica_detalhes', [ 'select' => '*', 'filters' => $filters, 'order' => 'valor.desc', 'limit' => $sample_limit ]);
    } catch (Exception $e) {
        header('Content-Type: application/json');
...
if (!empty($_GET['debug_vistos_rows']) && $_GET['debug_vistos_rows'] === '1') {
    ...
    try {
        $detalhes = $supabase->select('fdespesasfabrica_detalhes', [ 'select' => '*', 'filters' => $filters, 'order' => 'valor.desc', 'limit' => $limit ]);
    } catch (Exception $e) {
        header('Content-Type: application/json');
        echo json_encode([ 'success' => false, 'error' => 'Erro ao buscar detalhes', 'detail' => $e->getMessage() ], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        exit;
...
<?php
// Print debug marker about which metas table has data (if set)
echo $GLOBALS['METAS_TABLES_DEBUG'] ?? '';
?>

<div id="receita-content" class="p-6 ml-4">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl text-yellow-400">Acompanhamento financeiro - Fábrica</h2>
    ...
    <?php if ($periodo_selecionado): ?>
        ...
        <div class="bg-gray-800 rounded-lg p-4">
            ...
            <div class="overflow-x-auto">
                ...
                // Retorna o valor numérico de um detalhe (compatível com campos 'vlr_total' e 'valor')
                function getDetalheValor($detalhe) {
                    if (!is_array($detalhe)) return 0.0;
                    $v = null;
                    if (isset($detalhe['vlr_total'])) $v = $detalhe['vlr_total'];
                    elseif (isset($detalhe['valor'])) $v = $detalhe['valor'];
                    elseif (isset($detalhe['valor_total'])) $v = $detalhe['valor_total'];
                    elseif (isset($detalhe['total'])) $v = $detalhe['total'];
                    return floatval($v ?? 0);
                }
                ...
                // Processar RECEITAS
                // Totais para receitas (para mês aberto podemos separar recebido / pendente)
                $total_receita_recebida = 0.0;
                $total_receita_pendente = 0.0;

                foreach ($dados_receita as $linha) {
                    $categoria = trim(strtoupper($linha['categoria'] ?? ''));
                    $categoria_pai = trim(strtoupper($linha['categoria_pai'] ?? ''));

                    // Normalizar campo total do registro (algumas views usam total_receita_mes, outras usam total)
                    $total_linha = floatval($linha['total_receita_mes'] ?? $linha['total'] ?? $linha['valor'] ?? 0);

                    // Quando os dados vêm da view aberta, tentar extrair valor já recebido (vlr_pago / valor_pago)
                    $recebido = 0.0;
                    if (!empty($receita_origem_aberto)) {
                    ...
                    $linha['total_receita_mes'] = $total_linha; // garantir campo

                    $total_geral += $total_linha;
                    $total_receita_recebida += $recebido;
                    $total_receita_pendente += $pendente;
                ...
                foreach ($dados_despesa as $linha) {
                    ...
                    // Forçar categoria pai 'Z - SAIDA DE REPASSE' como saída não operacional
                    if ($categoria_pai === 'Z - SAIDA DE REPASSE') {
                    ...
                    elseif ($categoria_pai === '(-) DESPESAS DE VENDA' || $categoria_pai === 'DESPESAS DE VENDA' || $categoria_pai === 'DESPESA DE VENDA' || $categoria_pai === 'DESPESA VENDA') {
                        $despesa_venda[] = $linha;
                    }
                    // Tratar casos especiais: se a categoria for 'OUTROS INVESTIMENTOS' consideramos como INVESTIMENTO INTERNO (mantido)
                    elseif ($categoria === 'OUTROS INVESTIMENTOS' || $categoria_pai === 'OUTROS INVESTIMENTOS') {
                    ...
                    // Separar INVESTIMENTO INTERNO
                    elseif ($categoria_pai === 'INVESTIMENTO INTERNO') {
                        $investimento_interno[] = $linha;
                    }
                    // Separar INVESTIMENTO EXTERNO
                    elseif ($categoria_pai === 'INVESTIMENTO EXTERNO') {
                ...
                // Não imprimir comentário HTML quando o endpoint de debug JSON for solicitado,
                // porque o código a seguir chama header('Content-Type: application/json')
                // e qualquer saída prévia causa o warning "Headers already sent".
                if (empty($_GET['debug_categorias']) || $_GET['debug_categorias'] !== '1') {
                    ...
                    echo "Investimento Externo: " . count($investimento_externo) . " | ";
                ...
                if (!empty($_GET['debug_categorias']) && $_GET['debug_categorias'] === '1') {
                    ...
                    $resp = [
                        ...
                        'counts' => [
                            ...
                            'investimento_externo' => count($investimento_externo),
                ...
                <table class="w-full text-sm text-gray-300">
                    ...
                    <tbody>
                        ...
                        <?php 
                        // Priorizar meta explicitamente salva como 'RECEITA OPERACIONAL'.
                        // Fallbacks: 'RECEITA BRUTA' e 'RECEITAS OPERACIONAIS' para compatibilidade histórica.
                        $meta_receita_bruta = obterMeta('RECEITA OPERACIONAL');
                        if (empty($meta_receita_bruta) || $meta_receita_bruta == 0) {
                            $meta_receita_bruta = obterMeta('RECEITA BRUTA');
                        }
                        if (empty($meta_receita_bruta) || $meta_receita_bruta == 0) {
                            $meta_receita_bruta = obterMeta('RECEITAS OPERACIONAIS');
                        }
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="hover:bg-gray-800 text-gray-300">
                                                    ...
                                                    <td class="px-2 py-1 border-b border-gray-800 text-right font-mono text-orange-200 align-top">
                                                        <?= renderValorComPercent(getDetalheValor($detalhe)) ?>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                                            </tbody>
                    ...
                    <tbody class="subcategorias" id="sub-investimento-interno" style="display: none;">
                        <?php
                        // Agrupar por nome de categoria normalizado (uppercase, trimmed) para evitar linhas duplicadas
                        $investimento_por_categoria = [];
                        foreach ($investimento_interno as $linha) {
                            $rawCat = $linha['categoria'] ?? 'SEM CATEGORIA';
                            $catNorm = strtoupper(trim(preg_replace('/\s+/', ' ', $rawCat)));
                            if ($catNorm === '') $catNorm = 'SEM CATEGORIA';
                            $valor = floatval($linha['total_receita_mes'] ?? 0);
                            if (!isset($investimento_por_categoria[$catNorm])) {
                                $investimento_por_categoria[$catNorm] = [ 'display' => $rawCat, 'total' => 0.0 ];
                            }
                            $investimento_por_categoria[$catNorm]['total'] += $valor;
                        }

                        // Renderizar uma linha por categoria agrupada e, se houver detalhes, renderizar o bloco de detalhes uma única vez
                        foreach ($investimento_por_categoria as $catNorm => $dadosCat):
                            $categoria_individual = $dadosCat['display'];
                            $valor_individual = floatval($dadosCat['total']);
                        ...
                        ?>
                        <tr class="hover:bg-gray-700 text-gray-300 cursor-pointer" onclick="toggleDetalhes('investimento-interno-<?= md5($catNorm) ?>')">
                            <td class="px-3 py-2 border-b border-gray-700 pl-6">
                                <?= htmlspecialchars($categoria_individual) ?><?= renderCategoryUnreadBadge($categoria_individual) ?>
                            </td>
                            <td class="px-3 py-2 border-b border-gray-700 text-center">
                        ...
                        <?php
                            // detalhes por categoria (se existirem)
                            if (isset($detalhes_por_categoria[$catNorm])):
                        ?>
                        <tr class="detalhes-categoria" id="det-investimento-interno-<?= md5($catNorm) ?>" style="display: none;">
                            <td colspan="2" class="px-0 py-0 border-b border-gray-700">
                                <div class="bg-gray-900 rounded-lg m-2">
                                    <div class="px-4 py-2 bg-gray-800 rounded-t-lg text-xs text-gray-400 font-semibold">
                                        Lançamentos individuais - <?= htmlspecialchars($categoria_individual) ?><?= renderCategoryUnreadBadge($categoria_individual) ?>
                                    </div>
                                    <div class="px-2 py-2">
                                        <table class="w-full text-xs">
                                            <thead>
                    ...
                    <tbody>
                        <?php 
                        $meta_investimento_externo = obterMeta('INVESTIMENTO EXTERNO');
                        $percentual_investimento_externo = calcularPercentualMeta($total_investimento_externo, $meta_investimento_externo);
                        $cor_investimento_externo = obterCorBarra($percentual_investimento_externo, true); // é despesa
                        ?>
                        <tr class="hover:bg-gray-700 cursor-pointer font-semibold text-blue-400" onclick="toggleReceita('investimento-externo')">
                            <td class="px-3 py-2 border-b border-gray-700">
                                (-) INVESTIMENTO EXTERNO<?= renderParentUnreadBadge('INVESTIMENTO EXTERNO') ?>
                            </td>
                            <td class="px-3 py-2 border-b border-gray-700 text-center">
                                <div class="w-full">
                                    <div class="flex items-center justify-between text-xs mb-1">
                                        <span class="text-gray-400"><?= number_format($percentual_investimento_externo, 1) ?>%</span>
                                        <span class="text-gray-500">Meta: R$ <?= number_format($meta_investimento_externo, 0, ',', '.') ?></span>
                                    </div>
                                    <div class="w-full bg-gray-600 rounded-full h-2">
                                        <div class="bg-<?= $cor_investimento_externo ?>-500 h-2 rounded-full transition-all duration-300" style="width: <?= min($percentual_investimento_externo, 100) ?>%"></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-3 py-2 border-b border-gray-700 text-right font-mono">
                                <?= renderValorComPercent($total_investimento_externo) ?>
                            </td>
                        </tr>
                    </tbody>
                    
                    <!-- Detalhes dos INVESTIMENTOS EXTERNOS (ocultos inicialmente) -->
                    <tbody class="subcategorias" id="sub-investimento-externo" style="display: none;">
                        <?php foreach ($investimento_externo as $linha): ?>
                        <?php 
                        $categoria_individual = trim($linha['categoria'] ?? 'SEM CATEGORIA');
                        $valor_individual = floatval($linha['total_receita_mes'] ?? 0);
                        ...
                        ?>
                        <tr class="hover:bg-gray-700 text-gray-300 cursor-pointer" onclick="toggleDetalhes('investimento-externo-<?= md5($linha['categoria']) ?>')">
                            <td class="px-3 py-2 border-b border-gray-700 pl-6">
                                <?= htmlspecialchars($categoria_individual) ?><?= renderCategoryUnreadBadge($categoria_individual) ?>
                            </td>
                            <td class="px-3 py-2 border-b border-gray-700 text-center">
                        ...
                        <?php 
                            ...
                            if ($cat_key === '') $cat_key = 'SEM CATEGORIA';
                            if (isset($detalhes_por_categoria[$cat_key])): ?>
                        <tr class="detalhes-categoria" id="det-investimento-externo-<?= md5($linha['categoria']) ?>" style="display: none;">
                            <td colspan="2" class="px-0 py-0 border-b border-gray-700">
                                <div class="bg-gray-900 rounded-lg m-2">
                                    <div class="px-4 py-2 bg-gray-800 rounded-t-lg text-xs text-gray-400 font-semibold">
                                        Lançamentos individuais - <?= htmlspecialchars($linha['categoria']) ?><?= renderCategoryUnreadBadge($linha['categoria']) ?>
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-gray-700 cursor-pointer font-semibold text-indigo-400" onclick="toggleReceita('amortizacao')">
                            <td class="px-3 py-2 border-b border-gray-700">
                                (-) AMORTIZAÇÃO<?= renderParentUnreadBadge('AMORTIZAÇÃO') ?>
                            </td>
                            <td class="px-3 py-2 border-b border-gray-700 text-center">
                                <div class="w-full">
                                    <div class="flex items-center justify-between text-xs mb-1">
                    ...
                    <!-- Detalhes de AMORTIZAÇÃO (ocultos inicialmente) -->
                    <tbody class="subcategorias" id="sub-amortizacao" style="display: none;">
                        ...
                        <tr class="hover:bg-gray-700 text-gray-300 cursor-pointer" onclick="toggleDetalhes('amortizacao-<?= md5($linha['categoria']) ?>')">
                            ...
                            <td class="px-3 py-2 border-b border-gray-700 text-right font-mono text-indigo-300">
                                <?= renderValorComPercent($valor_individual) ?>
                            </td>
                        </tr>
                                        ...
                                        <table class="w-full text-xs">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="hover:bg-gray-800 text-gray-300">
                                                    ...
                                                    <td class="px-2 py-1 border-b border-gray-800 text-right font-mono text-indigo-200 align-top">
                                                        <?= renderValorComPercent(getDetalheValor($detalhe)) ?>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                    ...
                    <?php 
                    // Incluir INVESTIMENTO EXTERNO no impacto de caixa (subtrair interno e externo)
                    $impacto_caixa = (((((($total_geral - $total_tributos) - $total_custo_variavel) - $total_custo_fixo - $total_despesa_fixa - $total_despesa_venda) - $total_investimento_interno - $total_investimento_externo - ($total_amortizacao ?? 0)) - $total_saidas_nao_operacionais) - ($total_retirada_de_lucro ?? 0));

    ...
    <?php if ($periodo_selecionado): ?>
        ...
        <div class="bg-gray-800 rounded-lg p-4">
            ...
            <div class="mb-4 flex gap-2">
                ...
                <button onclick="abrirModalVerMetasFabrica()" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded transition-colors text-sm">
                    👀 Ver Metas Salvas
                </button>
            ...
            <script>
            function abrirModalVerMetasFabrica() {
                const periodo = encodeURIComponent('<?= $periodo_selecionado ?>');
                const url = '/modules/financeiro_bar/metas_api.php?action=fetch&sim=fabrica&periodo=' + periodo;
                console.debug('[metas][fabrica] fetch start', { url });
                fetch(url)
                    .then(r=>{
                        console.debug('[metas][fabrica] response status', r.status);
                        return r.json().catch(e=>{ console.error('[metas][fabrica] invalid json', e); throw e; });
                    })
                    .then(data => {
                        console.debug('[metas][fabrica] data', data);
                        if (!data.ok) { console.error('[metas][fabrica] returned not ok', data); return alert('Erro ao buscar metas'); }
                        renderModalVerMetasGenericF('fabrica', data.metas);
                    }).catch(e => { console.error('[metas][fabrica] fetch error', e); alert('Erro de rede (veja console)'); });
            }

            function renderModalVerMetasGenericF(sim, metas) {
                ...
                metas.forEach(m => {
                    ...
                    html += `<tr class="border-t"><td class="p-2">${escapeHtml(m.CATEGORIA)}</td><td class="p-2">${escapeHtml(m.SUBCATEGORIA)}</td><td class="p-2">${Number(m.META).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td class="p-2"><span id="meta-summary-${m.meta_key}">${escapeHtml(summary)}</span></td><td class="p-2"><button class="px-2 py-1 bg-indigo-600 text-white rounded" onclick="openMetaMonthsPickerFabrica('${m.meta_key}', ${JSON.stringify(assigned)})">Editar Meses</button></td></tr>`;
                });
            ...
            function saveMetaMonths(sim, meta_key) {
                ...
                console.debug('[metas][fabrica] save request', payload);
                fetch('/modules/financeiro_bar/metas_api.php?action=save_months', {
                    method: 'POST', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                }).then(r=>{
                    console.debug('[metas][fabrica] save status', r.status);
                    return r.json().catch(e=>{ console.error('[metas][fabrica] save invalid json', e); throw e; });
                }).then(res=>{ console.debug('[metas][fabrica] save response', res); if (res.ok) alert('Meses salvos'); else alert('Erro ao salvar'); }).catch(e=>{ console.error('[metas][fabrica] save error', e); alert('Erro de rede (veja console)'); });
            }

            function openMetaMonthsPickerFabrica(meta_key, assignedRaw) {
                console.debug('[metas][fabrica] openMetaMonthsPicker', meta_key, assignedRaw);
                ...
                inner += '</div>';
                inner += '<div class="mt-3 text-right"><button id="meta-save-btn-fabrica" class="px-4 py-2 bg-blue-600 text-white rounded">Salvar</button> <button onclick="closeMetaMonthsPickerFabrica()" class="px-4 py-2 bg-gray-600 text-white rounded">Cancelar</button></div>';
                inner += '</div>';

                const modal = document.createElement('div');
                modal.id = 'modalMetaPickerFabrica'; modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-70 flex items-center justify-center';
                modal.innerHTML = `<div class="bg-white rounded-lg p-4 max-w-3xl w-full mx-4">${inner}</div>`;
                document.body.appendChild(modal);

                document.getElementById('meta-save-btn-fabrica').addEventListener('click', function(){
                    ...
                    console.debug('[metas][fabrica] saving from picker', meta_key, meses);
                    const payload = { sim: 'fabrica', meta_key: meta_key, meses: meses };
                    fetch('/modules/financeiro_bar/metas_api.php?action=save_months', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
                        .then(r=>r.json()).then(res=>{
                            console.debug('[metas][fabrica] picker save response', res);
                            if (res.ok) {
                                const span = document.getElementById('meta-summary-' + meta_key);
                                if (span) span.textContent = (res.meses || []).join(', ');
                                closeMetaMonthsPickerFabrica(); alert('Meses salvos');
                            } else { alert('Erro ao salvar (veja console)'); }
                        }).catch(e=>{ console.error(e); alert('Erro de rede (veja console)'); });
                });
            }
            function closeMetaMonthsPickerFabrica() { const el = document.getElementById('modalMetaPickerFabrica'); if (el) el.parentNode.removeChild(el); }

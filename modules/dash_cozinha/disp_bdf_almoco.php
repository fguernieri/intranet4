...
<body class="bg-primary bg-opacity-70 min-h-screen flex items-center justify-center p-4">
  <form class="p-6 space-y-6 max-w-3xl w-full bg-white rounded-2xl shadow-md border border-primary" method="post" action="salvar_disp_bdf_almoco.php">
    <h1 class="text-2xl font-bold text-primary text-center">Disp de Cardápio BDF Almoço</h1>
    ...
    <div class="grid grid-cols-1 gap-4">
      <?php
      try {
        $stmt = $pdo->query("SELECT codigo_cloudify, nome_prato FROM ficha_tecnica WHERE ativo_bdf_almoco = 1 ORDER BY nome_prato");

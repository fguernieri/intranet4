...
<!doctype html>
    ...
    <head>
      ...
      <title>Pedido - <?= htmlspecialchars($filial, ENT_QUOTES) ?> (Sem Médias)</title>
      ...
      <main class="flex-1 p-6">
        <div class="max-w-7xl mx-auto w-full">
          <header class="mb-6">
            ...
            <h1 class="text-2xl font-bold">Pedido — <?= htmlspecialchars($filial) ?> (Sem Médias)</h1>
          ...
          <?php elseif ($mostrar_medias_estoques && !empty($medias_consumo)): ?>
            <div class="bg-green-700 text-white p-3 rounded mb-4">
              ...
              <br><small><strong>NOTA:</strong> As quantidades foram deixadas em branco intencionalmente. Para ver sugestões, use a página de pedido padrão.</small>
            </div>
          <?php endif; ?>
                ...
                <tbody id="insumo-list">
                  ...
                  <?php foreach ($insumos as $it):
                    ...
                    if ($mostrar_medias_estoques) {
                      ...
                      // MODIFICADO: campo quantidade sempre vazio
                      $qtde_input = '';
                    } else {
  ...
  document.querySelectorAll('input[name^="quantidade"]').forEach(i => { 
    try { 
      // Garantindo que todos os campos estejam vazios nesta versão
      i.value = ''; 
      i.autocomplete = 'off'; 
    } catch(e){} 
  });

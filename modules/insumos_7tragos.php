...
$stmt = $conn->prepare("
    ...
    LEFT JOIN (
        ...
        FROM vw_consumo_insumos7tragos_3m
        GROUP BY cod_insumo
...
foreach ($insumos as $row) {
    ...
    $insumosRecalculados[] = $row;
}
$insumos = $insumosRecalculados; // Substitui o array original pelo recalculado

if (empty($insumos)) {
    // A mensagem de aviso será exibida dentro da tabela se $insumos estiver vazio.
}
$stmt->close();
$conn->close();
        ...
        <table class="min-w-full text-xs mx-auto">
          ...
          <tbody id="insumo-body" class="divide-y divide-gray-700">
            <?php foreach ($insumos as $row):
              ...
              $estoqueColorClass = ''; // Adicionado para consistência
            ...
            <tr class="hover:bg-gray-700" data-cat="<?=$cat?>">
              ...
              <td class="p-2 text-center font-semibold <?=$estoqueColorClass?>">
                <?=$estoqueAtualDisplay?>
              </td>
...
`;
    ...
    document.getElementById('confirm-preview').onclick = async ()=>{
      ...
      try {
        const response = await fetch('salvar_pedido_7tragos.php', {
          method: 'POST',
          body: formData
        });
        if (response.ok) {
          ...
          alert(`Erro ao enviar o pedido para 7 Tragos: ${response.status} ${response.statusText}\n${errorText}`);
          btn.disabled = false;
          btn.innerText = 'Confirmar pedido';
        }
      } catch (error) {
